...
let unbanneduser = auditinfo.audit_log_entries[0].target_id;
let moderator = auditinfo.audit_log_entries[0].user_id;

// create log message
await lib.discord.channels['@0.1.1'].messages.create({
  channel_id: `${logchannel}`,
  content: `**Moderation Logs |** Unban #${casenumber}`,
  tts: false,
  embed: {
    type: 'rich',
    title: 'Ban',
    description: `A user was unbanned in ${guildinfo.name}!`,
    color: 0xcc2936,
    fields: [
      {
        name: 'User | 👤',
        value: `<@!${unbanneduser}> (${unbanneduser})`,
      },
      {
        name: 'Moderator | 🔒',
        value: `<@!${moderator}>`,
